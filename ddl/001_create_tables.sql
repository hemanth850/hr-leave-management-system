...
CREATE TABLE employee_roles (
    emp_role_id       NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    employee_id       NUMBER NOT NULL,
    role_code         VARCHAR2(30) NOT NULL,
    is_active         CHAR(1) DEFAULT 'Y' NOT NULL,
    assigned_at       TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,
    CONSTRAINT fk_emp_roles_emp FOREIGN KEY (employee_id) REFERENCES employees (employee_id),
    CONSTRAINT uq_emp_roles UNIQUE (employee_id, role_code),
    CONSTRAINT chk_emp_roles_role CHECK (role_code IN ('EMPLOYEE', 'MANAGER', 'HR', 'ADMIN')),
    CONSTRAINT chk_emp_roles_active CHECK (is_active IN ('Y','N'))
);
...
CREATE INDEX ix_employee_roles_emp ON employee_roles (employee_id, role_code, is_active);
